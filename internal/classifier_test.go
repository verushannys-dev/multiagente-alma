...
func TestResolver_SinDominio_NoDisponible(t *testing.T) {
	...
	if r.Dominio != nil {
		t.Fatalf("esperaba dominio nil (sin match alguno), obtuve %v", *r.Dominio)
	}
}

// Dominio real y bien identificado, pero sin agente ni API que lo
// resuelva (ej. rag hoy). De cara al consultante es no_disponible como
// cualquier otro, pero el Resultado interno mantiene el dominio
// poblado — a diferencia del caso sin match alguno (arriba).
func TestResolver_DominioSinResolver_NoDisponibleConDominio(t *testing.T) {
	c := &Classifier{catalogo: types.Catalogo{Agentes: []types.AgenteCatalogo{
		{ID: "rag", Estado: "sin_resolver"},
	}}}
	r := c.resolver(types.LLMOutput{Dominio: strPtr("rag")})
	if r.Resultado != "no_disponible" {
		t.Fatalf("esperaba 'no_disponible', obtuve %q", r.Resultado)
	}
	if r.Dominio == nil || *r.Dominio != "rag" {
		t.Fatalf("esperaba dominio 'rag' poblado, obtuve %v", r.Dominio)
	}
}
