...
func (c *Classifier) resolver(out types.LLMOutput) types.Resultado {
	...
	for _, agente := range c.catalogo.Agentes {
		if agente.ID == *out.Dominio {
			...
			case "sin_resolver":
				// Dominio real y bien identificado, pero hoy no lo
				// resuelve ni un agente ni la API (ej. contenido nuevo
				// del RAG). De cara al consultante se comporta igual
				// que no_disponible (mismo mensaje de cortesía, capa
				// de canal) — pero, a diferencia del no_disponible sin
				// match alguno, acá SÍ mantenemos Dominio poblado, para
				// que A8 pueda distinguir "no sabemos qué pidieron" de
				// "sabemos qué pidieron, y sabemos que falta resolverlo".
				return types.Resultado{Resultado: "no_disponible", Dominio: out.Dominio}
			default: // "via_api"
...
- Antes de concluir que un mensaje no corresponde a ningún dominio: si hay algún dominio parecido aunque no calce perfecto, preferí devolver motivo="ambiguedad_dominio" con una pregunta que intente confirmar o descartar ese dominio cercano (ej. ante "cómo llamar a una ambulancia", preguntá algo como "¿la necesitás para un traslado coordinado con anticipación?", con ese dominio como una opción) — en vez de rendirte directo a dominio=null. Reservá dominio=null para cuando de verdad no haya ningún candidato razonable.
